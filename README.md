...
原稿と同じフォルダの `dist/` に、**印刷用 PDF が生成**されます。あなたがすることは、もう書くことだけです。
...
出版・投稿時：`npx at-book` を走らせるだけ。あとは生成されたフォルダを開くだけです。
...
組版の対象にする原稿を指定します。**この項目が無いと何も生成されません。**
...
> **`dist/` は `.gitignore` に入れてください。** 生成物はいつでも作り直せるので、リポジトリに入れる必要はありません。`dist/` と書いておけば、どの階層の `dist/` にも効きます。
...
両方を設定すると、本文PDFと同時に `<原稿のフォルダ>/dist/<名前>-hyoshi.svg`（背幅・塗り足しガイド入り）が生成されます。
