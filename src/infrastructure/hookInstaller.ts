...
# at-book の実行方法を解決する。git フックは cwd がリポジトリルートで実行される。
#   1. ローカル依存 (node_modules/.bin) … npm 配布での通常経路。最速で、作品ごとに
#      バージョンが固定されるため、過去作を組み直しても組版が変わらない。
#   2. npx --no-install … workspaces でホイスティングされた場合の保険。
#      --no-install を付け、レジストリからの暗黙ダウンロードは行わせない。
#   3. PATH 上の at-book … 'npm link' で入れた従来ユーザーとの後方互換。
# どれも無ければ静かに失敗せず、復旧方法を案内して終了する。
if [ -x "./node_modules/.bin/at-book" ]; then
    ATBOOK="./node_modules/.bin/at-book"
elif npx --no-install at-book --version >/dev/null 2>&1; then
    ATBOOK="npx --no-install at-book"
elif command -v at-book >/dev/null 2>&1; then
    ATBOOK="at-book"
else
    MSG="[at-book] ⚠ at-book が見つかりません。このリポジトリで 'npm i -D at-book' を実行してください。自動生成をスキップします。"
